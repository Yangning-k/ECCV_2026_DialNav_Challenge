...
class GraphVlnAgentModel(Localization):
    ...
    def __init__(self, basepath, args=None, rank=0):
        ...
        # Inference must run in eval mode: the released code left the model in
        # train mode, so the panorama encoder had dropout active and produced
        # different embeddings on every scene build (non-deterministic
        # localization, and the scene cache could not be bit-identical).

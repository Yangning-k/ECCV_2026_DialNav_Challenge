...
class LANA(QuestionGeneration, AnswerGeneration):
    def __init__(self, basepath, args=None, rank=0, type='qg'):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(os.environ["DIALNAV_BERT_TOKENIZER_DIR"])
    ...
    def ask(self, scanIds, viewpoints):
        import os
        fixed_question = os.environ.get("FIXED_QUESTION")
        if fixed_question is not None:
            return [fixed_question] * len(viewpoints), None

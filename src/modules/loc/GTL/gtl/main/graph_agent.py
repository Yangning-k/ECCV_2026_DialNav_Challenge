...
class GraphVlnAgent(Seq2SeqAgent):
    ...
    def _build_model(self):
        ...
        # buffer
        self.scanvp_cands = {}
        # Scene-level cache: scan_id -> fully built GraphMap (all node embeddings).
        # Building the scene graph / panorama encoding is the expensive part and
        # depends only on the scene, not on the question, so we build it once per
        # scan and reuse it for every localization question.
        self.scene_gmap_cache = {}
        self.scene_cache_order = []
        # Bound the number of full scene graphs kept in GPU memory. Each built
        # scene holds its node embeddings (plus module-level topology caches) on
        # the GPU (~0.3-0.5 GB each in practice), so an unbounded cache OOMs on
        # long runs. The disk cache (GTL_SCENE_CACHE_DIR) remains the source of
        # truth; evicted scenes reload from disk in seconds.
        self.scene_cache_max = int(os.environ.get("GTL_SCENE_CACHE_MAX", "4"))
        # Per-viewpoint panorama features are also cached on the GPU and
        # unbounded; scene builds add one ~1 MB entry per viewpoint, so bound it.
        self.pano_feat_cache_order = []
        self.pano_feat_cache_max = int(os.environ.get("GTL_PANO_CACHE_MAX", "2000"))
        self.scene_cache_builds = 0
        self.scene_cache_hits = 0
        # Optional disk cache: rebuilt scenes are saved so later processes
        # (e.g. WTA threshold sweeps) can load them in seconds instead of
        # re-encoding every panorama. Set GTL_SCENE_CACHE_DIR to enable.
        self.scene_cache_dir = os.environ.get("GTL_SCENE_CACHE_DIR") or None
        if self.scene_cache_dir:
            os.makedirs(self.scene_cache_dir, exist_ok=True)
        self.scene_cache_disk_loads = 0
        self.scene_cache_disk_saves = 0
    ...
    def _panorama_feature_variable(self, obs):
        ...
        for i, ob in enumerate(obs):
            ...
            if scan_vp in self.pano_feat_cache:
                ...
                self.pano_feat_cache_order.append(scan_vp)
    ...
    def localize(self, texts, scanIds):
        ...
        conf, max_idx = probs.max(1)
        # Store per-item localization confidence for caller-side gating.
        self.last_loc_conf = conf.detach().cpu().tolist()
        self.last_loc_logits = logits.detach().cpu()
        self.last_loc_nodes = [list(gmap.node_positions.keys()) for gmap in gmaps]
        predicted = []
        for i, gmap in enumerate(gmaps):
            nodes = list(gmap.node_positions.keys())
            predicted.append(nodes[max_idx[i]])
    ...
    def prepare_gmaps(self, batch_items):
        if os.environ.get("GTL_NO_CACHE") == "1":
            batch_scans = [item['scanName'] for item in batch_items]
            batch_viewpoints = [item['viewpointId'] for item in batch_items]
        ...
        pending = []
        pending_seen = {}
        for scan, vp in zip(batch_scans, batch_viewpoints):
            if scan in self.scene_gmap_cache:
                gmaps.append(self.scene_gmap_cache[scan])
                self.scene_cache_hits += 1
                self._touch_scene(scan)
            elif scan in pending_seen:
                gmaps.append(pending_seen[scan])
            elif self.scene_cache_dir is not None:
                cache_path = os.path.join(self.scene_cache_dir, f"{scan}.pt")
                if os.path.exists(cache_path):
                    gmap = self._load_scene_from_disk(scan, cache_path)
                    self.scene_gmap_cache[scan] = gmap
                    self._touch_scene(scan)
                    self.scene_cache_disk_loads += 1
                    gmaps.append(gmap)
                    self.scene_cache_hits += 1
                    print(f"[GTL cache] loaded scene {scan} from disk ({len(gmap.node_positions)} nodes)")
                else:
                    gmap = GraphMap(vp)
                    gmaps.append(gmap)
                    pending.append((scan, gmap))
                    pending_seen[scan] = gmap
            else:
                gmap = GraphMap(vp)
                gmaps.append(gmap)
                pending.append((scan, gmap))
                pending_seen[scan] = gmap
        if pending:
            pending_scans = [s for s, _ in pending]
            pending_gmaps = [g for _, g in pending]
            self.build_all_map_embeddings(pending_scans, pending_gmaps, batch_size=1024)
            if os.environ.get("GTL_TRAIN_FREEZE_PANO") == "1":
                for _, gmap in pending:
                    for vp in gmap.node_embeds:
                        gmap.node_embeds[vp][0] = gmap.node_embeds[vp][0].detach()
            for scan, gmap in pending:
                self.scene_gmap_cache[scan] = gmap
                self._touch_scene(scan)
                self.scene_cache_builds += 1
                if self.scene_cache_dir is not None:
                    self._save_scene_to_disk(scan, gmap)
                    self.scene_cache_disk_saves += 1
                print(f"[GTL cache] built scene {scan} embeddings ({len(gmap.node_positions)} nodes) "
                      f"(total builds={self.scene_cache_builds}, hits={self.scene_cache_hits})")
        return gmaps

    def _touch_scene(self, scan):
        """Mark `scan` as most-recently-used and enforce the cache bound."""
        if scan in self.scene_cache_order:
            self.scene_cache_order.remove(scan)
        self.scene_cache_order.append(scan)
        evicted = False
        while len(self.scene_cache_order) > self.scene_cache_max:
            old = self.scene_cache_order.pop(0)
            self.scene_gmap_cache.pop(old, None)
            # Drop the evicted scene's per-viewpoint panorama features too.
            prefix = f"{old}_"
            for k in [k for k in self.pano_feat_cache if k.startswith(prefix)]:
                self.pano_feat_cache.pop(k, None)
                try:
                    self.pano_feat_cache_order.remove(k)
                except ValueError:
                    pass
            try:
                from gtl.models.graph_utils_for_loc import evict_graph_cache
                evict_graph_cache(old)
            except Exception:
                pass
            try:
                self.env.env.feat_db.clear_scan(old)
            except Exception:
                pass
            evicted = True
            print(f"[GTL cache] evicted scene {old} from memory cache")
        # Bound the panorama feature cache globally (LRU by insertion order).
        while len(self.pano_feat_cache) > self.pano_feat_cache_max:
            old_vp = self.pano_feat_cache_order.pop(0)
            self.pano_feat_cache.pop(old_vp, None)
            evicted = True
        if evicted:
            gc.collect()
            torch.cuda.empty_cache()

    def _scene_cache_path(self, scan):
        return os.path.join(self.scene_cache_dir, f"{scan}.pt")

    def _save_scene_to_disk(self, scan, gmap):
        data = {
            'node_positions': gmap.node_positions,
            'node_embeds': {k: [v[0].detach().cpu(), v[1]] for k, v in gmap.node_embeds.items()},
            'graph_dis': {k: dict(v) for k, v in gmap.graph._dis.items()},
            'graph_point': {k: dict(v) for k, v in gmap.graph._point.items()},
            'graph_visited': list(gmap.graph._visited),
            'node_step_ids': gmap.node_step_ids,
        }
        torch.save(data, self._scene_cache_path(scan))

    def _load_scene_from_disk(self, scan, cache_path):
        data = torch.load(cache_path, map_location='cpu', weights_only=False)
        gmap = GraphMap(None)
        gmap.node_positions = data['node_positions']
        gmap.node_embeds = {k: [v[0].cuda(), v[1]] for k, v in data['node_embeds'].items()}
        gmap.node_step_ids = data.get('node_step_ids', {})
        gmap.graph._dis = defaultdict(lambda: defaultdict(lambda: 95959595))
        for k, v in data['graph_dis'].items():
            gmap.graph._dis[k] = defaultdict(lambda: 95959595, v)
        gmap.graph._point = defaultdict(lambda: defaultdict(lambda: ""))
        for k, v in data['graph_point'].items():
            gmap.graph._point[k] = defaultdict(lambda: "", v)
        gmap.graph._visited = set(data['graph_visited'])
        return gmap

    def clear_scene_cache(self):
        """Clear the per-scene graph embedding cache to free GPU memory."""
        self.scene_gmap_cache = {}
        torch.cuda.empty_cache()

...
# Global caches
# _distance_cache = {}
_graph_cache = {}  # Cache for graph information per scan
_node_positions_cache = {}  # Cache for node positions per scan


def evict_graph_cache(scan):
    """Drop one scan from the module-level topology caches (frees memory)."""
    _graph_cache.pop(scan, None)
    _node_positions_cache.pop(scan, None)
...
class FloydGraph(object):
    ...
    def copy(self):
        ...
        """
        new_graph = FloydGraph()
        # Copy distance dictionary
        new_graph._dis = defaultdict(lambda: defaultdict(lambda: 95959595))
        for x in self._dis:
            for y in self._dis[x]:
                new_graph._dis[x][y] = self._dis[x][y]
        
        # Copy point dictionary
        new_graph._point = defaultdict(lambda: defaultdict(lambda: ""))
        for x in self._point:
            for y in self._point[x]:
                new_graph._point[x][y] = self._point[x][y]
        
        # Copy visited set
        new_graph._visited = self._visited.copy()
...
class GraphMap(object):
    ...
    # def _get_cached_distance(self, vp1, vp2):
    #     """
    #     Get distance between two viewpoints from cache
    #     """
    #     if (vp1, vp2) in _distance_cache:
    #         return _distance_cache[(vp1, vp2)]
    #     elif (vp2, vp1) in _distance_cache:
    #         return _distance_cache[(vp2, vp1)]
    #     else:
    #         print(f"Calculating distance between {vp1} and {vp2}")
    #         dist = calc_position_distance(self.node_positions[vp1], self.node_positions[vp2])
    #         _distance_cache[(vp1, vp2)] = dist
    #         _distance_cache[(vp2, vp1)] = dist
    #         return dist

    def update_graph(self, ob):
        """
        Updates the graph with a single observation.
        """
    ...
    def update_graph_batch(self, all_viewpoints_obs, scan_id):
        """
        Updates the graph with a batch of observations efficiently.
        If the graph for this scan is already cached, returns the cached version.
        
        Args:
            obs_batch (list): List of observations to process
            scan_id (str): The scan ID for caching
            
        Returns:
            FloydGraph: The graph for this scan
        """
        # Check if graph is already cached
        if scan_id in _graph_cache:
            self.graph = _graph_cache[scan_id].copy()
            self.node_positions = _node_positions_cache[scan_id].copy()
            # print(f" ---- Graph cache hit  {scan_id} - {len(list(self.graph._dis.keys()))}")
            return
        
        # print(f" ---- Graph cache miss  {scan_id}")

        # 1. Collect all unique viewpoints and their positions
        all_viewpoints = {}
        for ob in all_viewpoints_obs:
            all_viewpoints[ob['viewpoint']] = ob['position']
            for cand in ob['candidate']:
                all_viewpoints[cand['viewpointId']] = cand['position']
        
        # 2. Update node positions in batch
        self.node_positions.update(all_viewpoints)
        
        # 3. Collect all edges and their distances
        edges_to_add = []
        for ob in all_viewpoints_obs:
            curr_vp = ob['viewpoint']   
            for cand in ob['candidate']:
                vp_id = cand['viewpointId']
                if vp_id != curr_vp:
                    dist = calc_position_distance(ob['position'], cand['position'])
                    edges_to_add.append((curr_vp, vp_id, dist))
        
        # 4. Add all edges at once
        for curr_vp, vp_id, dist in edges_to_add:
            self.graph.add_edge(curr_vp, vp_id, dist)
        
        # 5. Update graph for all viewpoints in batch
        for ob in all_viewpoints_obs:
            self.graph.update(ob['viewpoint'])

        # Cache the graph
        _graph_cache[scan_id] = self.graph.copy()
        _node_positions_cache[scan_id] = self.node_positions.copy()
    ...
    def save_to_json(self):
        ...
        for vp, pos in self.node_positions.items():
            node_embed = self.get_node_embed(vp)
            nodes[vp] = {
                'location': pos,    # (x, y, z)
                'node_embed': node_embed.tolist(),
            }
    ...
    def to_cpu(self):
        """
        Convert all tensors in the graph map to CPU
        """
        cpu_gmap = GraphMap(self.start_vp)
        cpu_gmap.node_positions = self.node_positions.copy()
        cpu_gmap.node_embeds = {k: v.cpu() for k, v in self.node_embeds.items()}
        cpu_gmap.node_step_ids = self.node_step_ids.copy()
        cpu_gmap.node_stop_scores = self.node_stop_scores.copy()
        cpu_gmap.graph = self.graph.copy()
        return cpu_gmap

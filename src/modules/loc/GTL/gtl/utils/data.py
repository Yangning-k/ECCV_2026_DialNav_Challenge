...
class ImageFeaturesDB(object):
    def __init__(self, img_ft_file, image_feat_size, use_gpu=False, preload_all=False):
        ...
        self._feature_store = {}
        self._feature_store_order = []
        self._feature_store_max = int(os.environ.get("GTL_FEATDB_CACHE_MAX", "4000"))
    ...
    def get_image_feature(self, scan, viewpoint):
        ...
        if key in self._feature_store:
            ...
            with h5py.File(self.img_ft_file, 'r') as f:
                ...
                self._feature_store[key] = ft
                self._feature_store_order.append(key)
                self._enforce_bound()
        return ft

    def clear_scan(self, scan):
        """Drop all cached features for one scan (frees GPU memory)."""
        prefix = f"{scan}_"
        for key in [k for k in self._feature_store if k.startswith(prefix)]:
            self._feature_store.pop(key, None)
            try:
                self._feature_store_order.remove(key)
            except ValueError:
                pass

    def _enforce_bound(self):
        while len(self._feature_store) > self._feature_store_max:
            old = self._feature_store_order.pop(0)
            self._feature_store.pop(old, None)
